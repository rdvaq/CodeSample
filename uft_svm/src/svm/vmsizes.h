...
// n_instructions represent the actual number of insturctions + 1
...
static inline size_t vmsize_stacktrace_fun(int n_instructions) {
  struct VMFunction *fun;
  int t_num_instructions = 2 * n_instructions;
  return sizeof (*fun) + t_num_instructions * sizeof(fun->instructions[0]);
}

static inline size_t vmsize_fun(int n_instructions) {
    return vmsize_stacktrace_fun(n_instructions);
}

static inline size_t vmsize_fun_payload(struct VMFunction *fun) {
  return vmsize_fun(fun->num_insturctions);
}
