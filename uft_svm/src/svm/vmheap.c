...
#ifdef SMALLHEAP
  #define PAGESIZE 30000   // bytes in one page
...
static void scan_vmstate(struct VMState *vm) {
  ...
  Value* top_reg = (vm->curr_prog->nregs == 0) ? vm->reg_window + 1 : vm->reg_window + vm->curr_prog->nregs; 
  for (Value* curr_reg = vm->registers; curr_reg < top_reg; curr_reg++){
    scan_value(curr_reg);
  }
  /* zero out registers */
  int num_unscanned = REG_SIZE - (top_reg - vm->registers);
  memset(top_reg, 0, sizeof(*top_reg) * num_unscanned);
  ...
  Value* top_lit = vm->literals + vm->num_lits;
  for (Value* curr_lit = vm->literals; curr_lit < top_lit; curr_lit++) {
     scan_value(curr_lit);
  }
  // roots: all global-variable slots that are in use
  Value* top_glob = vm->globals + vm->num_globs;
  for (Value* curr_glob = vm->globals; curr_glob < top_glob; curr_glob++) {
     scan_value(curr_glob);
  }
  // roots: each function on the call stack
  struct Activation* top_act = vm->stack + vm->num_activations;
  for (struct Activation* curr_act = vm->stack; curr_act < top_act; curr_act++) {
     scan_activation(curr_act);
  }

  // root: the currently running function (which might not be on the call stack)
  vm->curr_prog = forward_function(vm->curr_prog);
...
extern void gc(struct VMState *vm) {
  assert(vm);
    ...
    gc_in_progress = true;
    int old_objects = count.current.objects;
    int old_bytes_requested = count.current.bytes_requested;
    ...
    fprint(stderr, "%d of %d objects holding %, of %, requested bytes survived\n",
            count.current.objects, old_objects, count.current.bytes_requested, old_bytes_requested);  // you fill in here
    fprintf(stderr, "Survival rate is %.1f%% of objects and %.1f%% of bytes\n",
            ((double)count.current.objects / old_objects) * 100.0, 
            ((double)count.current.bytes_requested / old_bytes_requested) * 100.0);  // you fill in here
    }
}

static void growheap(double gamma, int nlive) {
  int H = count.available.pages + count.current.pages;
//    printf("H: %d, gamma: %lf, nlisve: %d\n",H, gamma, nlive);

  bool grew = false;
  while (H < gamma * nlive) {
    acquire_available_page();
    grew = true;
    H = count.available.pages + count.current.pages;
  }

  if (grew && svmdebug_value("growheap"))
        fprintf(stderr, "Grew heap to %d pages\n", H);
  availability_floor = (H + 1) / 2;
}
