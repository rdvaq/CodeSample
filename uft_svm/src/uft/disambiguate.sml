...
struct
  ...
  val rec exp' : S.exp * environment -> X.exp = 
    ...
    let fun exp (S.LITERAL v)           = value v
          | exp (S.VAR x)               =
            (case referent (x, locals) of LOCAL => X.LOCAL x
                                        | PRIMITIVE p => etaExpand p
                                        | OTHER_GLOBAL => X.GLOBAL x)
          | exp (S.SET (x, e))          = 
             (case referent (x, locals) of LOCAL => X.SETLOCAL (x, exp e)
                                         | PRIMITIVE p => raise SetPrim "Trying to set primitive"
          ...
          | exp (S.APPLY (S.VAR x, es)) = 
             (case referent (x, locals) of LOCAL => X.FUNCALL (X.LOCAL x, map exp es)
                                         | PRIMITIVE p => X.PRIMCALL (p, map exp es)
                                         | OTHER_GLOBAL => X.FUNCALL (X.GLOBAL x, map exp es))   
          | exp (S.MAPPLY (srcloc, S.VAR x, es)) = 
             (case referent (x, locals) of LOCAL => X.MFUNCALL (srcloc, X.LOCAL x, map exp es)
                                         | PRIMITIVE p => X.MPRIMCALL (srcloc, p, map exp es)
                                         | OTHER_GLOBAL => X.MFUNCALL (srcloc, X.GLOBAL x, map exp es) )                                               
          | exp (S.APPLY (e, es)) = X.FUNCALL (exp e, map exp es)
          | exp (S.MAPPLY (_, e, es)) = X.FUNCALL (exp e, map exp es)
  ...
  local
    ...
    and exp' (X.FUNCALL (f, args))      = S.APPLY (exp f, map exp args)
      | exp' (X.MFUNCALL (_, f, args))  = S.APPLY (exp f, map exp args)
      | exp' (X.PRIMCALL (p, es))       = S.APPLY (S.VAR (P.name p), map exp es)
      | exp' (X.MPRIMCALL (_, p, es))    = S.APPLY (S.VAR (P.name p), map exp es)
