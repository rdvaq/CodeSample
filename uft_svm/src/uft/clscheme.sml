...
structure ClosedScheme = struct
  ...
  datatype exp
    ...
    | MPRIMCALL of srcloc * Primitive.primitive * exp list
...
struct
  ...
  fun exp (C.CAPTURED i) = S.APPLY (S.VAR "CAPTURED-IN", [S.LITERAL (S.INT i), S.VAR "$closure"])
    | exp (C.CLOSURE ((formals, body), captured)) =
      S.APPLY (S.VAR "mkclosure", [S.LAMBDA (("$closure" :: formals), exp body), SU.list (map exp captured)])
    ...
    | exp (C.MFUNCALL (_, f, es))      = S.APPLY (exp f, map exp es)
    | exp (C.PRIMCALL (p, es))     = S.APPLY (S.VAR (P.name p), map exp es)
    | exp (C.MPRIMCALL (_, p, es)) = S.APPLY (S.VAR (P.name p), map exp es)
