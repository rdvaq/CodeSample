...
struct
  ...
  local  (* projection *)
    fun exp (X.LITERAL v)        = succeed (F.LITERAL (KNProject.value v))
      ...
      | exp (X.MFUNCALL (loc, e, es))  = Impossible.exercise "project marked fun"
      | exp (X.PRIMCALL (p, es)) = curry F.PRIMCALL p <$> exps es
      | exp (X.MPRIMCALL (loc, p, es))  = Impossible.exercise "project marked primfun"
