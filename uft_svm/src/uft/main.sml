...
structure Main = struct
  ...
  fun run f name stream = f name (stream, TextIO.stdOut)
  ...
  fun custom_comp f g x = f x (g x)

  fun tx f []    = run f "-" TextIO.stdIn 
    | tx f paths = errorApp (custom_comp (run f) openIn) paths
 
  val _ = tx : (string -> TextIO.instream * TextIO.outstream -> unit Error.error) ->
               string list -> unit Error.error
    
  fun translationOf spec file =
    case String.fields (fn c => c = #"-") spec
      of [from, to] =>
           (case (Languages.find from, Languages.find to, file)
              of (SOME from, SOME to, "-") => UFT.translate (from, to, "stdin")
               | (SOME from, SOME to, _) => UFT.translate (from, to, file)
               | (NONE, _, _) => die ("I don't recognize language `" ^ from ^ "`")
               | _ => die ("I don't recognize language `" ^ to ^ "`"))
       | _ => usage()
  val _ = translationOf : string -> string -> TextIO.instream * TextIO.outstream -> unit Error.error
