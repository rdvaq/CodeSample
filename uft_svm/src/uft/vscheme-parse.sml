...
struct
  ...
  (* ignore the srcloc by default *)
  val one = let fun finish (Sx.MARK (sx, _)) = finish sx
                  | finish sx = sx
            in  finish <$> one
            end
  ...
  val firstMark : srcloc option parser =  (* consumes no input *)
    let fun ofOne (Sx.MARK (_, loc)) = SOME loc
          | ofOne (Sx.LIST sxs) = ofList sxs
          | ofOne _ = NONE
        and ofList (x :: xs) = (case ofOne x of NONE => ofList xs | loc => loc)
          | ofList [] = NONE
    in  P.ofFunction (fn ltokens => SOME (Error.OK (ofList ltokens), ltokens))
  end
 
  fun maybeMarkApply (SOME loc) = curry3 S.MAPPLY loc
    | maybeMarkApply NONE = curry S.APPLY  
  ...
  fun freeIn exp y =
    let fun member y [] = false
        ...
        fun has_y (S.LITERAL _) = false
          ...
          | has_y (S.MAPPLY (_, e, es))   = List.exists has_y (e::es)
  ...
  val exp = P.fix (fn exp =>
    ...
    in     bracket "set"       (curry  S.SET <$> name <*> exp)
       ...
       <|> oflist (maybeMarkApply <$> firstMark <*> exp <*> many exp)
