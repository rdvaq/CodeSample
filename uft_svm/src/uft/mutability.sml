...
struct
  ...
  fun hasLambda (X.FUNCALL (f, args))      = anyLambda (f :: args)
    | hasLambda (X.MFUNCALL (_, f, args))  = anyLambda (f :: args)
    | hasLambda (X.PRIMCALL (p, es))       = anyLambda es
    | hasLambda (X.MPRIMCALL (_, p, es))   = anyLambda es
  ...
  fun setsLocal (X.FUNCALL (f, args))      = anySet (f :: args)
    | setsLocal (X.MFUNCALL (_, f, args))  = anySet (f :: args)
    | setsLocal (X.PRIMCALL (p, es))       = anySet es
    | setsLocal (X.MPRIMCALL (_, p, es))   = anySet es
